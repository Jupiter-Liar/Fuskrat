...
function openPopup(imageUrl) {
	...
	// Create and append the about button
	const aboutButton = document.createElement("button");
	aboutButton.id = "about-button";
	// aboutButton.innerHTML = "&#9432;"; // Information symbol
	aboutButton.innerHTML = "i"; // Information symbol
	aboutButton.onclick = (event) => {
		// <-- Add 'event' parameter here
		event.stopPropagation(); // Now 'event' is defined and has stopPropagation()
		openAboutDiv(popupDiv);
	};

	// Append the close button and image to the popup div
	stickyHeader.appendChild(closeButton);
	stickyHeader.appendChild(aboutButton);
	popupDiv.appendChild(stickyHeader);
	popupDiv.appendChild(img);

	// Function to open the about div
	function openAboutDiv(targetPopupDiv) {
		// Accepts popupDiv as an argument
		//console.log("openAboutDiv called.");
		//console.log("targetPopupDiv:", targetPopupDiv); // Should show the actual #popup-container element

		// Check if about div already exists
		const existingAboutDiv = targetPopupDiv.querySelector("#about-div");
		//console.log("existingAboutDiv check:", existingAboutDiv); // Expected: null

		if (existingAboutDiv) {
			//console.log("About div already open. (This should NOT be seen based on your feedback).");
			return; // Keep this commented out for now, just to be absolutely sure
		}

		// Create the about div container
		const aboutDiv = document.createElement("div");
		aboutDiv.id = "about-div";
		//console.log("aboutDiv created:", aboutDiv); // Should show the newly created div

		// Apply styles (important for visibility if appended)
		//aboutDiv.style.position = "fixed";
		//aboutDiv.style.top = "50%";
		//aboutDiv.style.left = "50%";
		//aboutDiv.style.transform = "translate(-50%, -50%)";
		//aboutDiv.style.zIndex = "10000";
		//aboutDiv.style.textAlign = "center";
		//aboutDiv.style.padding = "20px";
		//aboutDiv.style.backgroundColor = "white"; // Make sure it has a background to be seen
		//aboutDiv.style.color = "black"; // Ensure text is visible
		//aboutDiv.style.minWidth = "200px"; // Give it some guaranteed size
		//aboutDiv.style.minHeight = "150px"; // Give it some guaranteed size
		//aboutDiv.style.borderRadius = "8px";
		//aboutDiv.style.boxShadow = "0 4px 8px rgba(0, 0, 0, 0.2)";

		// Create Heading
		const aboutHeading = document.createElement("h2");
		aboutHeading.textContent = "About Fuskrat";
		aboutDiv.appendChild(aboutHeading);
		//console.log("Heading appended."); // ADDED LOG

		const aboutDetails = document.createElement("div");
		aboutDetails.id = "about-details";
		aboutDiv.appendChild(aboutDetails);

		// Create Close button for About div
		const aboutCloseButton = document.createElement("button");
		aboutCloseButton.id = "about-close-button";
		aboutCloseButton.innerHTML = "&times;"; // The "×" symbol
		aboutCloseButton.onclick = () => {
			//console.log("About Close button clicked. Removing aboutDiv.");
			aboutDiv.remove();
		};
		aboutDiv.appendChild(aboutCloseButton);
		//console.log("Close button appended."); // ADDED LOG

		// ... (License, Linktree, Buy Me a Coffee sections - KEEP THESE AS IS) ...
		const licenseSpan = document.createElement("span");
		//licenseSpan.innerHTML = "This extension was created by Jupiter Liar and is licensed ";
		licenseSpan.innerHTML = "This extension was created by Jupiter Liar";
		const licenseSVG = document.createElement("img");
		licenseSVG.src = chrome.runtime.getURL("svg/CC_BY-SA_icon.svg");
		licenseSVG.alt = "License Icon";
		licenseSpan.appendChild(licenseSVG);
		// licenseSpan.innerHTML += ".";
		aboutDetails.appendChild(licenseSpan);

		const linktreeDiv = document.createElement("div");
		const linktreeLink = document.createElement("a");
		linktreeLink.href = "https://linktr.ee/jupiterliar";
		const linktreeSpan = document.createElement("span");
		linktreeSpan.textContent = "Linktree";
		const linktreeSVG = document.createElement("img");
		linktreeSVG.src = chrome.runtime.getURL("svg/linktree.svg");
		linktreeSVG.alt = "Linktree Icon";
		linktreeLink.appendChild(linktreeSpan);
		linktreeLink.appendChild(linktreeSVG);
		linktreeDiv.appendChild(linktreeLink);
		aboutDetails.appendChild(linktreeDiv);

		const buyMeACoffeeDiv = document.createElement("div");
		const buyMeACoffeeLink = document.createElement("a");
		buyMeACoffeeLink.href = "https://buymeacoffee.com/jupiterliar";
		const buyMeACoffeeSpan = document.createElement("span");
		buyMeACoffeeSpan.textContent = "Buy Me a Coffee";
		const buyMeACoffeeSVG = document.createElement("img");
		buyMeACoffeeSVG.src = chrome.runtime.getURL("svg/buy-me-a-coffee.svg");
		buyMeACoffeeSVG.alt = "Buy Me a Coffee Icon";
		buyMeACoffeeLink.appendChild(buyMeACoffeeSpan);
		buyMeACoffeeLink.appendChild(buyMeACoffeeSVG);
		buyMeACoffeeDiv.appendChild(buyMeACoffeeLink);
		aboutDetails.appendChild(buyMeACoffeeDiv);

		// --- TEMPORARILY DISABLE CLICK-OUTSIDE-TO-CLOSE FOR DEBUGGING ---
		aboutDiv.addEventListener("click", (event) => {
			event.stopPropagation();
		});

		targetPopupDiv.addEventListener("click", (event) => {
			//console.log("popupDiv click event. Target:", event.target);
			if (aboutDiv && !aboutDiv.contains(event.target)) {
				console.log("Click outside aboutDiv. Removing aboutDiv.");
				aboutDiv.remove();
			}
		});
		// --- END TEMPORARY DISABLE ---

		//console.log("Attempting to append aboutDiv to targetPopupDiv..."); // ADDED LOG
		targetPopupDiv.appendChild(aboutDiv);
		//console.log("aboutDiv appended to targetPopupDiv. Checking if it's in DOM..."); // ADDED LOG

		// A small delay to allow DOM update, then check
		//setTimeout(() => {
		//	const appendedCheck = targetPopupDiv.querySelector("#about-div");
		//	console.log("After setTimeout check, aboutDiv is in DOM:", !!appendedCheck, appendedCheck); // ADDED LOG
	...
	function handleFuskButtonClick(popupDiv, location) {
		...
		const selectedSpanIndex = nodes.findIndex(
			(node) => node.nodeType === Node.ELEMENT_NODE && node.hasAttribute("selected")
		);
		...
		const suffix = nodes
			.slice(selectedSpanIndex + 1, hexContainerIndex)
			.map((node) => node.textContent || "")
			.join("");
